...
CREATE INDEX employeeid ON orders(
    employeeid
);
...
CREATE INDEX shippersorders ON orders(
    shipvia
);
...
CREATE INDEX categoriesproducts ON products(
    categoryid
);
...
CREATE INDEX supplierid ON products(
    supplierid
);
...
CREATE INDEX orderid ON order_details(
    orderid
);
...
CREATE INDEX productid ON order_details(
    productid
);
...
CREATE OR REPLACE VIEW quarterly_orders AS
    ...
    WHERE
        customers.customerid = orders.customerid
        AND to_date(orders.orderdate,
        'MM/DD/YYYY') BETWEEN TO_DATE('19970101',
        'YYYYMMDD')
        AND TO_DATE('19971231',
        'YYYYMMDD');
...
CREATE VIEW order_subtotals AS
    SELECT
        order_details.orderid,
        SUM(order_details.unitprice*quantity*(1-discount)/100*100) AS subtotal
    FROM
        order_details
    GROUP BY
        order_details.orderid;
...
CREATE OR REPLACE VIEW product_sales_for_1997 AS
    ...
    WHERE
        ...
        AND orders.orderid = order_details.orderid
        AND products.productid = order_details.productid
        AND to_date(orders.shippeddate,
        'MM/DD/YYYY') BETWEEN TO_DATE('19970101',
        'YYYYMMDD')
        AND TO_DATE('19971231',
        'YYYYMMDD')
...
CREATE OR REPLACE VIEW sales_totals_by_amount AS
    ...
    FROM
        ...
        orders
    WHERE
        orders.orderid = order_subtotals.orderid
        AND customers.customerid = orders.customerid
        AND (order_subtotals.subtotal >2500)
        AND (to_date(orders.shippeddate,
        'MM/DD/YYYY') BETWEEN TO_DATE('19970101',
        'YYYYMMDD')
        AND TO_DATE('19971231',
...
INSERT INTO orders (
    ...
    84.81,
    'LILA-Supermercado',
    'Carrera 52 con Ave. Bolívar #65-98 Llano Lar',
    'Barquisimeto',
    'Lara',
    '3508',
    'Venezuela'
...
CREATE PROCEDURE SalesByCategory
    @CategoryName varchar2(15), @OrdYear varchar2(4) = '1998'
AS
...
WHERE OD.OrderID = O.OrderID 
	...
	AND SUBSTRING(CONVERT(varchar2(22), O.OrderDate, 111), 1, 4) = @OrdYear
GROUP BY ProductName
...
ALTER TABLE CustomerCustomerDemo
	...
	(
		[CustomerTypeID]
	) REFERENCES [xxms].[CustomerDemographics] (
		[CustomerTypeID]
	)
;
